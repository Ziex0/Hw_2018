...
public:
	...
	bool OnGossipSelect(Player * pPlayer, Creature * pCreature, uint32 sender, uint32 uiAction)
	{
		...
		case 2: // Vip account one month - unlocked
		...
		case 3: // Vip account three months - unlocked
			if (pPlayer->GetSession()->GetSecurity() >= 1)
			{
		...
		case 4: // Vip account three months - unlocked
			if (pPlayer->GetSession()->GetSecurity() >= 1)
			{
				pPlayer->GetSession()->SendAreaTriggerMessage("You're already VIP");
				pPlayer->CLOSE_GOSSIP_MENU();
	...
	void CheckConditions(Player* player)
	{
		...
		if (!select)
		{
			...
			sLog->outError(LOG_FILTER_GENERAL,"NOTE: Automatic VIP accounts script just removed the VIP rights for player %s (accountId: '%u'). There are no "
				"records in the vip_conditions table for this player anymore, so the changes will take effect once the player logout.", 
