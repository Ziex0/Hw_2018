...
void Enchant(Player* player, Item* item, uint32 enchantid)
{
    ...
    if (!enchantid)
    {
        player->GetSession()->SendNotification("Something went wrong in the code. It has been logged for developers and will be looked into, sorry for the inconvenience.");
         sLog->outError(LOG_FILTER_GENERAL,"Enchant NPCenchantid is NULL, something went wrong here!");
        return;
    }
		
    item->ClearEnchantment(PERM_ENCHANTMENT_SLOT);
    item->SetEnchantment(PERM_ENCHANTMENT_SLOT, enchantid, 0, 0);
    player->GetSession()->SendNotification("|cff800080%s |cffFF0003succesfully enchanted!", item->GetTemplate()->Name1.c_str());
}
