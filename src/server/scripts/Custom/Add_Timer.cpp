...
#define REWARD_AMOUNT 3 // how much vp gives if player meets online time
			...
			for (SessionMap::const_iterator iter = smap.begin(); iter != smap.end(); ++iter)
			{
				if (Player* player = iter->second->GetPlayer())
				{
					...
					if(extention.online_time >= EQUIRED_ONLINE_TIME)
					{
						...
						ChatHandler(player->GetSession()).PSendSysMessage("Congratulations your online that has been reached, you have got %u vp", REWARD_AMOUNT);
					}
				}
			}
		}
