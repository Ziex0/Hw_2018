...
void teleportPlayerToGuildHouse(Player *player, Creature *_creature)
{
	if (player->GetGuildId() == 0)
	 {
	 //if player has no guild
	 _creature->MonsterWhisper(MSG_NOTINGUILD, LANG_UNIVERSAL, player);
	 return;
 }
 ...
 {
	//if player in combat
	_creature->MonsterWhisper(MSG_INCOMBAT, LANG_UNIVERSAL, player);
	return;
 }
		...
		if (getGuildHouseCoords(player->GetGuildId(), x, y, z, map))
 {
		...
		player->TeleportTo(map, x, y, z, 0.0f);
				...
				_creature->MonsterWhisper(MSG_NOGUILDHOUSE, LANG_UNIVERSAL, player);
}
...
bool showBuyList(Player *player, Creature *_creature, uint32 showFromId = 0)
{
 ...
 _creature->MonsterWhisper(MSG_NOFREEGH, LANG_UNIVERSAL, player);
...
bool isPlayerHasGuildhouse(Player *player, Creature *_creature, bool whisper = false)
{
			...
			if (result)
	{
			if (whisper)
			{
			 ...
			 _creature->MonsterWhisper(msg, LANG_UNIVERSAL, player);
			}
...
void buyGuildhouse(Player *player, Creature *_creature, uint32 guildhouseId)
{
	 ...
	 bool token = ConfigMgr::GetBoolDefault("GuildHouse.TokenOrGold", false);
	 int cost = ConfigMgr::GetBoolDefault("GuildHouse.Cost", false);

	 if (player->GetMoney() < COST_GH_BUY)
	{
	 //show how much money player need to buy GH (in gold)
	 char msg[100];
	 sprintf(msg, MSG_NOTENOUGHMONEY, COST_GH_BUY - ConfigMgr::GetBoolDefault("GuildHouse.Cost", false));
	 _creature->MonsterWhisper(msg, LANG_UNIVERSAL, player);
	 return;
	}
		...
		//player already have GH
		 ...
		 if (result)
		{
	 _creature->MonsterWhisper(MSG_GHOCCUPIED, LANG_UNIVERSAL, player);
				return;
		}
	 ...
	 player->ModifyMoney(ConfigMgr::GetIntDefault("GuildHouse.BuyCost", false) * 10000);

	 //player->DestroyItemCount(token, cost, true);
	_creature->MonsterWhisper(MSG_CONGRATULATIONS, LANG_UNIVERSAL, player);
 ...
 if (isPlayerHasGuildhouse(player, _creature))
	{
		 ...
		 player->ModifyMoney(ConfigMgr::GetIntDefault("GuildHouse.SellCost", false) * 10000);

 //display message e.g. "here your money etc."
	 char msg[100];
	 sprintf(msg, MSG_SOLD, ConfigMgr::GetIntDefault("GuildHouse.SellCost", false));
	 _creature->MonsterWhisper(msg, LANG_UNIVERSAL, player);
	}
}
