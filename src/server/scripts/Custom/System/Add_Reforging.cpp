/*Coded By ????
Updated For Last Revision Trinity By: Amir_Cinderella <Amir.Cinderella@Gmail.Com>
*/
    ...
    void OnLogin(Player* player, bool /*firstLogin*/)
    {
        ...
        if (result)
        {
            ...
            do
            {
                ...
                if(sObjectMgr->_itemFakeStatStore.find(lowGUID) != sObjectMgr->_itemFakeStatStore.end() && sObjectMgr->_itemFakeStatStore.erase(lowGUID)>= maxReforges)
                {
                    if(sent_error_message.find(lowGUID) != sent_error_message.end())
                        continue;
                   sLog->outError(LOG_FILTER_GENERAL,"bg.battleground","Too many reforges for item lowGUID %u, ignoring extra reforges.", lowGUID);
        ...
        case SELECT_STAT_INCREASE:
            ...
            if (Item* InvItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
            {
                if(IsReforgable(InvItem, player))
                {
                    ...
                    for (uint32 i = 0; i < proto->StatsCount; ++i)
                    {
                        if(hasfakestats && sObjectMgr->_itemFakeStatStore.find(i) != sObjectMgr->_itemFakeStatStore.end())
        ...
        if(InvItem->IsEquipped() && InvItem->GetOwnerGUID() == player->GetGUID())
        {
            ...
            if(proto->Quality != ITEM_QUALITY_HEIRLOOM && proto->StatsCount > 0)
            {
                for (uint32 i = 0; i < proto->StatsCount; ++i)
                {
                    ...
                    if(sObjectMgr->_itemFakeStatStore.find(InvItem->GetGUIDLow()) != sObjectMgr->_itemFakeStatStore.end())
                    {
                        if(sObjectMgr->_itemFakeStatStore.find(i) == sObjectMgr->_itemFakeStatStore.end())
