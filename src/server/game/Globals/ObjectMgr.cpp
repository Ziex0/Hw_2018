...
void ObjectMgr::LoadCreatureOutfits()
{
    ...
    if (!result)
    {
       sLog->outError(LOG_FILTER_GENERAL, ">> Loaded 0 creature outfits. DB table `creature_template_outfits` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError(LOG_FILTER_GENERAL, ">> Creature entry %u in `creature_template_outfits`, but not in `creature_template`!", entry);
            continue;
        }
        ...
        if (!rEntry)
        {
            sLog->outError(LOG_FILTER_GENERAL, ">> Creature entry %u in `creature_template_outfits` has incorrect race (%u).", entry, uint32(co.race));
            continue;
        }
        ...
        switch (co.gender)
        {
            ...
            default:
                sLog->outError(LOG_FILTER_GENERAL, ">> Creature entry %u in `creature_template_outfits` has invalid gender %u", entry, uint32(co.gender));
                continue;
        }
    ...
    sLog->outError(LOG_FILTER_GENERAL, ">> Loaded %u creature outfits in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadReloadPetTransmogs()
{
    m_PetTransmogContainer.clear();

    QueryResult result = WorldDatabase.PQuery("SELECT id, name, price FROM pet_transmogs");
    if (result)
    {
        do
        {
            ...
            PetTransmog transmogentry;

            transmogentry.displayid = fields[0].GetUInt32();
            transmogentry.name      = fields[1].GetString();
            transmogentry.price     = fields[2].GetUInt32();

            m_PetTransmogContainer.push_back(transmogentry);
        }
        while (result->NextRow());
    }
}
