...
public:
    ...
    struct warlock_botAI : public bot_minion_ai
    {
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            if (CURSE_OF_THE_ELEMENTS && GC_Timer <= diff && dist < 30 && Rand() < 15 &&
            ...
            if (CORRUPTION && GC_Timer <= diff && dist < 30 && Rand() < 25 &&
                !opponent->HasAura(CORRUPTION, me->GetGUID()) &&
                doCast(opponent, CORRUPTION))
                return;

            if (HAUNT && Haunt_cd <= diff && GC_Timer <= diff && dist < 30 && Rand() < 25 &&
            ...
            if (GC_Timer <= diff && dist < 30 && Rand() < 15 && !Afflicted(opponent))
            {
            ...
            if (CONFLAGRATE && conflagarate_cd <= diff && GC_Timer <= diff && dist < 30 && Rand() < 35 &&
            ...
            if (CHAOS_BOLT && chaos_bolt_cd <= diff && GC_Timer <= diff && dist < 30 && Rand() < 50 &&
            ...
            if (SHADOW_BOLT && GC_Timer <= diff && dist < 30 &&
