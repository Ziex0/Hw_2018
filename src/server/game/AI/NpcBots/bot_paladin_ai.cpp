...
public:
    ...
    struct paladin_botAI : public bot_minion_ai
    {
       ...
       /*else */if (spellInfo->GetAllEffectsMechanicMask() & (1<<MECHANIC_SNARE) || 
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            //buff
            if (SEAL_OF_COMMAND && Rand() < 20 && GC_Timer <= diff && !me->HasAura(SEAL_OF_COMMAND) &&
        ...
        void Counter(uint32 diff)
        {
            ...
            Unit* target = Repentance_Timer < 25000 ? FindCastingTarget(20, 0, false, REPENTANCE) : NULL;
            if (target)
        ...
        void ApplyClassDamageMultiplierMelee(int32& damage, SpellNonMeleeDamage& /*damageinfo*/, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool& crit) const
        {
            ...
            if (!crit)
            {
                ...
                if (lvl >= 45 && (spellInfo->GetRecoveryTime() == SPELLCATEGORY_JUDGEMENT || spellInfo->GetSpellSpecific() == SPELL_SPECIFIC_JUDGEMENT))
                    aftercrit += 18.f;
            ...
            if (lvl >= 40 &&
                (spellInfo->GetRecoveryTime() == SPELLCATEGORY_JUDGEMENT ||
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (spellId == BLESSING_OF_KINGS || spellId == BLESSING_OF_MIGHT ||
                spellId == BLESSING_OF_WISDOM || spellId == BLESSING_OF_SANCTUARY)
            {
  ...
  /*Damage*/SEAL_OF_COMMAND, HOLY_WRATH, EXORCISM, CRUSADER_STRIKE, JUDGEMENT,
