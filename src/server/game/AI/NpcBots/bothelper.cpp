...
std::string BotHelper::GetLocaleStringForTextID(std::string& textValue, uint32 textId, int32 localeIdx)
{
    ...
    {
        sLog->outError(LOG_FILTER_GENERAL, "botgiver:GetLocaleStringForTextID:: unknown text id: %u!", uint32(textId));
        return textValue;
    }
    ...
    if (localeIdx < 0)
    {
        sLog->outError(LOG_FILTER_GENERAL, "botgiver:GetLocaleStringForTextID:: unknown locale: %i! Sending default locale text...", localeIdx);
        return textValue;
    }
