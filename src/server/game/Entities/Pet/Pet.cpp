...
void Pet::Update(uint32 diff)
{
    ...
    checkdisplayid ? displayidtimer += diff : displayidtimer = 0;

    if (displayidtimer >= 60000)
    {
    ...
    switch (petType)
    {
        ...
        default:
        {
            switch (GetEntry())
            {
				...
				case 40000: // CUSTOM pet
                {
                    if (!pInfo)
                    {
                        SetCreateMana(28 + 10*petlevel); // Mana multiplier based on your pet's level
                        SetCreateHealth(28 + 30*petlevel); // Health multiplier based on your pet's level
                    }
                    int32 bonus_dmg = int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW)* 0.3f); // Spell (SHADOW) bonus damage done
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 4 - petlevel) + bonus_dmg)); // MinDamage
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 4 + petlevel) + bonus_dmg)); // MaxDamage
