...
void Channel::JoinChannel(Player* player, std::string const& pass)
{
    ...
    if (_announce && !player->GetSession()->GetSecurity())
    {
        WorldPacket data;
...
void Channel::KickOrBan(Player const* player, std::string const& badname, bool ban)
{
    ...
    if (!playersStore[good].IsModerator() && !player->GetSession()->GetSecurity())
    {
        WorldPacket data;
    ...
    if (!player->GetSession()->GetSecurity() && changeowner && good != _ownerGUID)
    {
        WorldPacket data;
        MakeNotOwner(&data);
        SendToOne(&data, good);
    ...
    else if (!player->GetSession()->GetSecurity())
    {
        WorldPacket data;
...
void Channel::Password(Player const* player, std::string const& pass)
{
    ...
    if (!playersStore[guid].IsModerator() && !player->GetSession()->GetSecurity())
    {
        WorldPacket data;
        MakeNotModerator(&data);
        SendToOne(&data, guid);
...
void Channel::SetOwner(Player const* player, std::string const& newname)
{
    ...
    if (!player->GetSession()->GetSecurity() && guid != _ownerGUID)
    {
        WorldPacket data;
        MakeNotOwner(&data);
        SendToOne(&data, guid);
