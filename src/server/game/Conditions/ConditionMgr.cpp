...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
		...
		case CONDITION_GUILD_LEVEL:
        {
            ...
            {
               sLog->outError(LOG_FILTER_SQL,"sql.sql", "Guildlevel condition has invalid option (%u), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
               sLog->outError(LOG_FILTER_SQL,"sql.sql", "Guildlevel condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_ITEM_EQUIPPED:
        {
