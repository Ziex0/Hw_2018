...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
    ...
    if (!code.empty())
    {
        ...
        else if (guid == _player->GetGUID())
        {
            ...
            {
                 sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleGossipSelectOptionOpcode - Player (GUID: %u) do not have a helper on gossip select.", uint32(GUID_LOPART(guid)));
                return;
            }
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        uint16 lvl = itr->second->getLevel();
...
void WorldSession::HandleAddFriendOpcodeCallBack(PreparedQueryResult result, std::string const& friendNote)
{
    ...
    if (result)
    {
        ...
        if (!AccountMgr::IsPlayerAccount(GetSecurity()) || sWorld->getBoolConfig(CONFIG_ALLOW_GM_GROUP) || AccountMgr::IsPlayerAccount(AccountMgr::GetSecurity(friendAccountId, realmID)))
        {
            if (friendGuid)
            {
