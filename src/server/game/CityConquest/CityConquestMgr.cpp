	...
	if (city->active)
	{
		...
		for (l = city->spawns.begin(); l != city->spawns.end(); ++l)
		{
			...
			if (city->controlled == 1)
			{
				...
				if(CreatureData const *cinfo = spawn->creature->GetCreatureData())
				{
					const_cast<CreatureData*>(cinfo)->dbData = spawn->faction_A;
					const_cast<CreatureData*>(cinfo)->dbData = spawn->faction_A;
				}
			...
			else if (city->controlled == 0)
			{
				...
				if(CreatureData const *cinfo = spawn->creature->GetCreatureData())
				{
					const_cast<CreatureData*>(cinfo)->dbData = spawn->faction_H;
					const_cast<CreatureData*>(cinfo)->dbData = spawn->faction_H;
				}
...
void CityConquestMgr::LoadCities()
{
	...
	if (result)
	{
		do
		{
			...
			// sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: City Loaded - %s", fields[3].GetString()); -- causes crash during startup (LordPsyan)
...
void CityConquestMgr::RegisterCreature(Creature * creature, int type)
{
	...
	if (type == 0)
	{
		sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: Creature Registered - %u", creature->GetEntry());
		result
	...
	{
		sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: City Spawner attmpts to register - %u", creature->GetEntry());
		result
		...
		if (result)
		{
			sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: City Spawner passed first check - %u", creature->GetEntry());
			Field *fields = result->Fetch();
			Cities::iterator l;
			for (l = cities.begin(); l != cities.end(); ++l)
			{
				City *ct = *l;
				sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: City Spawner browses through cities - %u", creature->GetEntry());
				if (ct->id == fields[0].GetInt32())
				{
					if (creature->GetEntry() == fields[1].GetInt32())
					{
						sLog->outInfo(LOG_FILTER_SERVER_LOADING,"CQM: City Spawner Registered - %u", creature->GetEntry());
						if (!ct->summonerSet)
						{
